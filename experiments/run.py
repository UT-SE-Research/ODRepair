...
#GUAVA_JAR = SCRIPT_DIR + '/libs/guava-30.1.1-jre.jar'
...
def runAnalysis(project_name, project_sha, test_fqn, field_fqn,
                downloads_dir=_DOWNLOADS_DIR, results_dir=_RESULTS_DIR,
                xmls_dir=XMLS_DIR, tool_jar=TOOL_JAR):
    client_result_dir = results_dir + '/' + project_name + '/' + \
        project_sha + '/' + test_fqn
    if not os.path.isdir(client_result_dir):
        os.makedirs(client_result_dir)
    cwd = os.getcwd()
    os.chdir(downloads_dir + '/' + project_name)
    analysis_log = client_result_dir + '/analysis.txt'
    xml_file = xmls_dir + '/' + project_name + '/' + project_sha + '/' + test_fqn + \
        '/' + field_fqn + '.xml'
    start_time = time.time()
    # map list resetters
    ...
    sub.run('java -jar ' + tool_jar + \
            ...
            stdout=open(analysis_log, 'a'), stderr=sub.STDOUT)
    extractResetters(project_name, project_sha, test_fqn, field_fqn)
    end_time = time.time()
    insertTimeInLog(start_time, end_time, analysis_log)
    os.chdir(cwd)

def extractResetters(project_name, project_sha, test_fqn, field_fqn,
                     results_dir=_RESULTS_DIR, resetter_list_dir=RESETTER_LIST_DIR):
    output_dir = resetter_list_dir + '/' + project_name + '/' + \
        project_sha + '/' + test_fqn
    if not os.path.isdir(output_dir):
        os.makedirs(output_dir)
    analysis_log_file = results_dir + '/' + project_name + '/' + \
        project_sha + '/' + test_fqn + '/analysis.txt'
    resetters = []
    with open(analysis_log_file, 'r') as fr:
        lines = fr.readlines()
    for i, l in enumerate(lines):
        if l.startswith('[INFO] Potential Resetter: '):
            resetter_fqn = l.strip().split()[-1]
            if '<init>' in resetter_fqn or '<clinit>' in resetter_fqn:
                continue
            # workaround
            if '$' in resetter_fqn:
                resetter_fqn = resetter_fqn.split('$')[0] + '.' + \
                    resetter_fqn.split('$')[-1].split('.')[-1]
            if resetter_fqn not in resetters:
                resetters.append(resetter_fqn)
        elif ', PUTSTATIC method: ' in l:
            resetter_fqn = l.strip().split()[-1]
            if '<init>' in resetter_fqn or '<clinit>' in resetter_fqn:
                continue
            # workaround
            ...
            if resetter_fqn not in resetters:
                resetters.append(resetter_fqn)
    resetters.append('org.mockito.Mockito.reset([Ljava/lang/Object;)V')
    resetters_json_file = output_dir + '/resetters.json'
    with open(resetters_json_file, 'w') as fw:
        json.dump(resetters, fw, indent=2)
...
def runMinePrims(project_name, project_sha, test_fqn, field_fqn,
    ...
    end_time = time.time()
    insertTimeInLog(start_time, end_time, analysis_log)
    genRandoopPrimsSpecFile(project_name, project_sha, test_fqn, field_fqn)
    os.chdir(cwd)

def runMinePrimsAllClasses(project_name, project_sha, test_fqn, field_fqn,
                           downloads_dir=_DOWNLOADS_DIR, results_dir=_RESULTS_DIR,
                           tool_jar=TOOL_JAR):
    client_result_dir = results_dir + '/' + project_name + '/' + \
        project_sha + '/' + test_fqn
...
def genRandoopPrimsSpecFile(project_name, project_sha, test_fqn, field_fqn,
...
def runRandoopTestGen(project_name, project_sha, test_fqn, field_fqn,
...
def runRandoopTestGenOnOneResetter(resetter_fqn, project_name, project_sha,
    ...
    with open(method_list_file, 'w') as fw:
        ...
        for tm in target_methods:
            fw.write(tm + '\n')
        # --- Target Classes
        # randoop_fmt_resetter_classes_fqn = convertJVMSigToClassFQN(resetter_fqn)
        # print('Randoop Format Classes Resetter FQN: ' + \
        #       randoop_fmt_resetter_classes_fqn)
        # fw.write(randoop_fmt_resetter_classes_fqn)
    client_result_dir = results_dir + '/' + project_name + '/' + \
        project_sha + '/' + test_fqn    
...
def runRandoopTestGenClassLevel(project_name, project_sha, test_fqn, field_fqn,
                                downloads_dir=_DOWNLOADS_DIR,
                                results_dir=_RESULTS_DIR,
                                gen_tests_dir=GEN_TESTS_DIR):
    print ('Copy jars ... '  + str(datetime.datetime.now()))
...
def runExp():
    ...
    for case in all_cases:
        ...
        if project_sha in finished_shas_fields and \
           field_fqn in finished_shas_fields[project_sha]:
            ...
            shutil.copytree(_RESULTS_DIR + '/' + project_name + '/' + project_sha + \
                            '/' + finished_shas_fields[project_sha][field_fqn],
        ...
        runAnalysis(project_name, project_sha, test_fqn, field_fqn)

        runMinePrims(project_name, project_sha, test_fqn, field_fqn)
        #runMinePrimsAllClasses(project_name, project_sha, test_fqn, field_fqn)

        runFindRefClasses(project_name, project_sha, test_fqn, field_fqn)

        runRandoopTestGen(project_name, project_sha, test_fqn, field_fqn)
        runRandoopTestGenClassLevel(project_name, project_sha, test_fqn, field_fqn)
        ...
        if field_fqn not in finished_shas_fields[project_sha]:
            finished_shas_fields[project_sha][field_fqn] = test_fqn
